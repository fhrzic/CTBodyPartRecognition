...
class SeriesContainer():
    ...
    def __init__(self, 
        ...
        Args:
            ...
            * number_of_processes, int, if not None, during the series extraction multiprocessing will be used
        """
        # Set verbose flag
    ...
    def _process_series(self, args: tuple = None )->dict:
        """
        Function which extracts series data from given dir and id. It
        returns dict containing all necessary information 
        Args:
            * args, tuple, containing _dir_name and _series_id:
                - _dir_name, path to the directory where series is stored,
                - _series_id, id of the series 
        Returns:
            * dictionary which contains all informations regarding series 
        """
        # Obtain data
        try:
            _dir_name, _series_ID = args
        except:
            print(f"Mising arguments in porocessing series! Aborting this series")
            return 0
    ...
    def obtain_data_from_series(self, path_dir: str = None, mode = 'a')->list:
        ...
        if self.verbose:
            print("--------------------------------------------------------------------------------------")
        if self.number_of_processes == None:
            for _dir_name in _series_dirs:
                ...
                for _i, _series_ID in enumerate(_series_IDs):
                    ...
                    _series_dict = self._process_series((_dir_name, _series_ID))
                    
                    # Add to export list
                    if _series_dict:
                        _main_storage.append(_series_dict)
        else:
            with multiprocessing.Pool(processes=self.number_of_processes) as _pool:
                _results = []
                for _dir_name in _series_dirs:
                    # Obtain all series IDs inside _dir
                    _series_IDs = sitk.ImageSeriesReader.GetGDCMSeriesIDs(_dir_name)
                    ...
                    for _i, _series_ID in enumerate(_series_IDs):
                        ...
                        if self.verbose:
                            print(f"Working on series {_i+1}/{len(_series_IDs)}")
                        _result = _pool.apply_async(self._process_series, args=((_dir_name, _series_ID),))   
                        _results.append(_result)

                # Collect the results
                _final_results = [_result.get() for _result in _results]
            # Filter invalids outputs
            for _series_dict in _final_results:
                if _series_dict:
                    _main_storage.append(_series_dict)
        # Save/Return Info        
        if self.verbose:
    ...
    def _export_series(self, 
        ...
        """
        Method which exports series to the folder. It accepts args necessary for the export

        Args:
            * series, dict, dict containing all necessarry data for series export.
        ...
        for _series in series:
            ...
            _series_description = self.cleanup_name(_series['series_tags']["0008|103e"][0])
            _base_name ="_".join([_main_name, _series_description, _series['series_ID']])

            # Cleanup
            _base_name = self.cleanup_name(_base_name)

            if self.verbose:
                print(f"Exporting data {_base_name}")
    ...
    def export_data_to_directory(self, 
        ...
        if self.number_of_processes == None:
            # Single processing
            self._export_series(self.main_data, export_dir_path,export_id, export_nifti, export_npy)
        else:
            # Multi porcessing
            _sublists = self.divide_list(self.main_data, self.number_of_processes)
            for _series in _sublists:
                _p = Process(target=self._export_series, args=(_series, export_dir_path, export_id, export_nifti, export_npy))
                _p.start()
            _p.join()   
    ...
    def _export_images(self, 
                       ...
                       histogram_reduction_params: dict = {"search_region": 400,
                                                            ...
                                                            "upper_bound": 1500}):
        """
        Method to export images to the folder. It accepts args necessary for the export
        Args:
            * series, dict, dict containing all necessarry data for series export.
        ...
        for _series in series:
            ...
            if self.verbose:
                print(f"Exporting images {_base_name}")
    ...
    def export_images_to_directory(self, export_dir_path:str = None,
        ...
        if self.number_of_processes == None:
            # Single processing
            self._export_images(self.main_data, export_dir_path, export_id, resample_spacing, lower_bound, upper_bound, histogram_reduction_params)
        else:
            # Multi porcessing
            _sublists = self.divide_list(self.main_data, self.number_of_processes)
            for _series in _sublists:
                _p = Process(target=self._export_images, args=(_series, export_dir_path, export_id, resample_spacing, lower_bound, upper_bound, histogram_reduction_params))
                _p.start()
            _p.join()  

    def cleanup_name(self, name:str = None)->str:
        '''
        Clean name to get rid of special characters which may cause issues during saving
        
        Parameters
        ----------
        name:  <str>
            Input name with potential special characters/space
        ...
        '''
        _name = name.lower()
        _name = re.sub('[^a-zA-Z0-9 _\n\.]', '_', _name)
        _name = re.sub(' ', '_', _name)
        return _name

    def divide_list(self, input_list:list, number_of_sublists: int)->list:
        """
