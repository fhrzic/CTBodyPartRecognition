...
import torch
from Utils.models_NMDD_BCH_SAROS import *
import numpy as np
import datetime
import pandas as pd
import xlsxwriter
...
class model_training_app:
    ...
    def init_model(self):
        ...
        """
        assert self.model_params.model_name in ['eff0', 'eff3', 'dense161', 'vit_h_14', 'eff5'], f"Model not implemented!"
        ...
        if self.model_params.model_name == 'eff3':
        ...
        if self.model_params.model_name == 'eff5':
            _model = EfficientNetB3(pretrained = self.model_params.pretrained, number_of_classes = self.model_params.number_of_output_classes)
            freeze_model_base(_model, freeze = False, seq = False)

        if self.model_params.model_name == 'dense161':
            _model = DenseNet161(pretrained = self.model_params.pretrained, number_of_classes = self.model_params.number_of_output_classes)
            freeze_model_base(_model, freeze = False, seq = False)

        if self.model_params.model_name == 'vit_h_14':
            _model = vit_h_14(pretrained = self.model_params.pretrained, number_of_classes = self.model_params.number_of_output_classes)
