...
class model_training_app:
    ...
    def init_model(self):    
        ...
        """
        assert self.model_params.model_name in ['eff0', 'eff3', 'dense161', 'vit_h_14', 'eff5', 'resNet50'], f"Model not implemented!"

        print(f"USING MODEL: {self.model_params.model_name}, WEIGHTS: UNFROZEN")

        # Currently implemented 
        if self.model_params.model_name == 'resNet50':
    ...
    def freeze_unfreeze_model(self, freeze: bool = True):
        ...
        """
        # Check if resnet
        if self.model_params.model_name in ['resNet50']:
            _seq = True
        else:
            _seq = False
