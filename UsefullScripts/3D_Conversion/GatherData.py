...
def clean_directories_based_on_images(image_paths, directories_to_check):
    ...
    """
    valid_directories = [os.path.dirname(image).split("NMDID_PATCHES")[-1] for image in image_paths]
    print(valid_directories[0:10])
    print("############")
    print(directories_to_check[0:5])
    _cnt = 0
    _cnt_2 = 0
    for dir_path in directories_to_check:
        _new_dir_path = dir_path.split("NMDID_PATCHES")[-1]
