...
class model_training_app:
    ...
    def init_model(self):
        ...
        """
        assert self.model_params.model_name in ['VoxCNN_Patches', 'ResNet', 'R3D_18', 'MC3_18', 'R2plus1d_18', 'ViT_3D'], f"Model not implemented!"

        print(f"USING MODEL: {self.model_params.model_name}, WEIGHTS: UNFROZEN")

        # Currently implemented 
        if self.model_params.model_name == 'VoxCNN_Patches':
            _model = VoxCNN_Patches(input_shape = self.model_params.input_shape['input_shape'], output_classes = self.model_params.number_of_output_classes, head = self.model_params.input_shape["head"])
    ...
    def freeze_unfreeze_model(self, freeze: bool = True):
        ...
        """
        # Set seq
        if self.model_params.model_name in ['VoxCNN_Patches']:
            _seq = False
    ...
    def init_augmentation_model(self):
        ...
        if self.model_params.augmentation_model == 'GRAY_Augmentation':
            self.aug_model = TransformToGray_AugmentedPatches(number_of_channels = self.model_params.input_channels)
            self.aug_model_color_only = TransformToGray_SimplePatches(number_of_channels = self.model_params.input_channels)
                        
        if self.model_params.augmentation_model == 'GRAY_Simple':
            self.aug_model = TransformToGray_SimplePatches(number_of_channels = self.model_params.input_channels)
            self.aug_model_color_only = TransformToGray_SimplePatches(number_of_channels = self.model_params.input_channels)
    ...
    def get_loss(self, index, batch, predictions_storage, true_storage, augmentation = True):
        ...
        # Obtain predictions
        _sliced_input_data = torch.split(_input_data, 27, dim=0)
        _prediction = []

        for _data in _sliced_input_data:
            _data = _data.to(self.device)
            _p = self.model(_data)
            _prediction.append(_p)
        
        _prediction = torch.cat(_prediction, dim=0)
        # Caluclate loss
    ...
    def model_predict_from_dl_plain_prediction(self, input_data_loader)->dict:
        ...
        for _batch in input_data_loader:
            ...
            for _data in _sliced_input_data:
                ...
                _prediction = torch.cat(_prediction, dim=0)
            # Convert to _numpy
            _predictions_numpy = _prediction.cpu().detach().numpy()
