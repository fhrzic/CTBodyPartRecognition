...
import Utils.config_Patches_NMDD_BCH_SAROS as cfg
import pandas as pd
import numpy as np
import random
import os
...
class ProcessData: 
    ...
    def __init__(self, sample:dict = None, 
        ...
        # Images   
        self.image = np.load(_image_path)
        # Set dimension
    ...
    def __pad_image(self, image_desired_size:int):
        ...
        """
        # Set desired shape
        _desired_shape = (image_desired_size, image_desired_size, image_desired_size)

        # Obtain paddings thorugh all dimensions
        _padding = [(max((_desired_shape[_i] - self.image.shape[_i]) // 2, 0),
                max(_desired_shape[_i] - self.image.shape[_i] - (_desired_shape[_i] - self.image.shape[_i]) // 2, 0))
               for _i in range(3)]

        # Apply padding
        self.image = np.pad(self.image, _padding, mode='constant', constant_values=0)
...
#**********************************************************************#
class NMDD_BCH_SAROS_dataset_3D:
    """
    ...
    def __getitem__(self, indx):
        ...
        _image = (_image - (-1024.0)) / (1500.0 - (-1024.0))
    ...
    def __obtain_structured_data(self)->list:
        ...
        for _index, _image_path in enumerate(_images_paths):
            ...
            # Check image path
            _image_path = os.path.join(_dir, "reduced.npy")
        ...
        for _path in _dir_paths:
            ...
            _image_path = os.path.join(_path, "reduced.npy")
...
_train
        ...
        Output:
            * Torch DataLoader
    """
    _ds = NMDD_BCH_SAROS_dataset_3D(dataset_params)
