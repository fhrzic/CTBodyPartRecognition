...
class model_training_app:
    ...
    def init_model(self):
        ...
        """
        assert self.model_params.model_name in ['eff0'], f"Model not implemented!"

        print(f"USING MODEL: {self.model_params.model_name}, WEIGHTS: UNFROZEN")

        # Currently implemented eff0
        if self.model_params.model_name == 'eff0':
            _model = EfficientNetB0(pretrained = self.model_params.pretrained, number_of_classes = self.model_params.number_of_output_classes)
    ...
    def freeze_unfreeze_model(self, freeze: bool = True):
        ...
        """
        # Check if resnet
        if self.model_params.model_name in ['res18','res34','res50','res101', 'res152', 'incept_v3']:
            _seq = True
        else:
            _seq = False

        if self.model_params.model_name in ['vit_b_16', 'vit_b_32', 'vit_l_16', 'vit_l_32', 'vit_h_14']:
            _seq = 'vit'
    ...
    def init_augmentation_model(self):
        ...
        """
        if self.model_params.augmentation_model == 'RGB_Augmentation':
            self.aug_model = TransformToRGB_Augmented()
            self.aug_model_color_only = TransformToRGB_Simple()          
        

        if self.model_params.augmentation_model == 'GRAY_Augmentation':
            self.aug_model = TransformToGray_Augmented()
            self.aug_model_color_only = TransformToGray_Simple()
            
        if self.model_params.augmentation_model == 'RGB_Simple':
            self.aug_model = TransformToRGB_Simple()
            self.aug_model_color_only = TransformToRGB_Simple()      
            
        if self.model_params.augmentation_model == 'GRAY_Simple':
            self.aug_model = TransformToGray_Simple()
            self.aug_model_color_only = TransformToGray_Simple()
    ...
    def model_predict_from_dl_full_body(self, input_data_loader,
                                  input_image_path:str = None, 
                                  input_xlsx_path = None,
                                  input_cheatsheet_path = None,
                                  label_type: str = "cluster_remaped",
                                  threshold = 0.5)->list:
        """
        Function is predicting results for the given dataloader and stores them in a dict
        ...
        Return:
            * list, list of output_dicts, each dict with following information: case_id, image to plot, predicted, true and 
            all label data.
        """
        ...
        # Model to eval
        self.model.eval()
        for _i, _batch in enumerate(input_data_loader):
            print(f"{_i}/{len(input_data_loader.dataset)}")
            # Obtain image prediction
            _image, _label, _cases = _batch
            _input_data = _image.to(self.device, non_blocking = True)
            _input_data = self.aug_model_color_only(_input_data)
            _predictions = self.model(_input_data)	
            _predictions = _predictions.to('cpu')
            
            # Go through every prediction and case
            for _img, _prediction, _case in zip(_input_data, _predictions, _cases):
                ...
                # Export dict to list:
                _main_list.append({
                "image_path": os.path.join(input_image_path, f"{_case}_reducted_image.png"),
                "id": _case,
                "prediction":  _prediction,
                "true": _label_output,
                "original_labels": _labels,
                "area": _area_output,
                "image": _img.cpu().permute(1, 2, 0).numpy()
                })
